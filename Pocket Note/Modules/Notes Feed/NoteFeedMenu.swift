...
protocol NoteFeedMenu: Menu {
    func setNotebookButton(target: PNNotesFeedViewController?, currentNotebook: Notebook?, navigationItem: UINavigationItem)
}

extension NoteFeedMenu {
    /**
     Sets the navigation title as the notebook name and the options for editing the notebook.
     - Parameter editNotebookInteractor: A  `PNEditNotebookInteractor` instance for editing the notebook.
    ...
    func setNotebookButton(target: PNNotesFeedViewController?, currentNotebook: Notebook?, navigationItem: UINavigationItem) {
        if currentNotebook != nil {
            ...
            notebookButton.addTarget(target, action: #selector(PNNotesFeedViewController.showNotebookActions), for:.touchUpInside)
        } else {
