...
struct HTMLDecoder {
    static func decode(htmlString: String) -> NSAttributedString? {
        ...
        html.beginEditing()
        html.enumerateAttribute(NSAttributedStringKey.font, in: NSMakeRange(0, html.length), options: .init(rawValue: 0)) { (value, range, _) in
            if let font = value as? UIFont {
                let resizedFont = font.withSize(PNNoteTypographyContants.normalFontSize)
                html.addAttribute(NSAttributedStringKey.font, value: resizedFont, range: range)
            }
        }
        let resizedFont = UIFont.init(name: PNNoteTypographyContants.noteNormalFont, size: PNNoteTypographyContants.normalFontSize)
        
        if html.length != 0 {
            html.addAttribute(NSAttributedStringKey.font, value: resizedFont as Any, range: NSMakeRange(html.length-1, 1))
        }
